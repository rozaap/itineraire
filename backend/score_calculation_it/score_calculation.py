...
def ponderer_all():
    ...
    for index, row in network_final.iterrows():
        value_ombre = abs(row["portion_om"]*8-800)
        ...
        fresh_value = value_ombre+value_brumi+value_assise+value_borne_font+value_fontaine+value_parc+value_sanitaire+value_vegetation+ value_temp
        network_final.loc[index, "fresh_value"] = fresh_value
    
    col_freshness = ["u", "v", "fresh_value"] 
